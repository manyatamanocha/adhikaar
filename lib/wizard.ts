...
const hi: Record<QuestionId, Question> = {
  ...
  court: {
    id: "court", prompt: "क्या अदालत ने बैंक को भुगतान करने से रोका है?",
    help: "केवल अदालती मामला होने का मतलब यह नहीं कि भुगतान रुका है। 'हाँ' तभी चुनें जब भुगतान रोकने वाला आदेश हो। यक़ीन न हो तो बैंक से पूछें।",
    options: [{ value: "no", label: "ऐसा कोई आदेश नहीं है" }, { value: "yes", label: "हाँ, एक आदेश भुगतान रोकता है" }, unknownHi],
  },
  nominee: {
...
const kn: Record<QuestionId, Question> = {
  ...
  court: {
    id: "court", prompt: "ಬ್ಯಾಂಕ್ ಪಾವತಿಸದಂತೆ ನ್ಯಾಯಾಲಯ ತಡೆದಿದೆಯೇ?",
    help: "ಕೇವಲ ನ್ಯಾಯಾಲಯದ ಪ್ರಕರಣ ಇರುವುದರಿಂದ ಪಾವತಿ ನಿಂತಿದೆ ಎಂದಲ್ಲ. ಪಾವತಿ ತಡೆಯುವ ಆದೇಶ ಇದ್ದಾಗ ಮಾತ್ರ 'ಹೌದು' ಆಯ್ಕೆಮಾಡಿ. ಖಚಿತವಿಲ್ಲದಿದ್ದರೆ ಬ್ಯಾಂಕಿಗೆ ಕೇಳಿ.",
    options: [{ value: "no", label: "ಅಂತಹ ಆದೇಶ ಇಲ್ಲ" }, { value: "yes", label: "ಹೌದು, ಒಂದು ಆದೇಶ ಪಾವತಿಯನ್ನು ತಡೆಯುತ್ತದೆ" }, unknownKn],
  },
  nominee: {
...
export function resolve(a: Answers, locale: Locale = "en"): Resolution {
  ...
  // Asked on every path -- 9 Sep 2026, later pass. A "have not been to the
  // bank yet" entry used to skip this question outright (a reader who is not
  // sure the money exists cannot answer whether a judge has restrained its
  // payment) and carried the assumption forward on the verdict instead, as a
  // hard caveat. That box read as the product describing its own plumbing
  // rather than answering the reader, so the question is asked directly
  // instead: it costs one extra question on that entry path, in exchange for
  // never having to state an assumption on a legal verdict.
