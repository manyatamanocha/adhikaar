...
export default async function ConfirmDetails({ searchParams }: {
  ...
  const steps = [
    ...(a.court !== "no" ? [restricted ? t.stepRestrictedYes : t.stepRestrictedAsk] : []),
    ...(a.will !== "no" && a.nominee === "no" ? [will ? t.stepWillYes : t.stepWillAsk] : []),
    ...(a.heirs !== "agree" ? [t.stepDispute] : []),
    ...(a.bankType === "unknown" || !a.bankType ? [t.stepBankTypeUnknown] : []),
    ...(a.amount === "unknown" || a.amount === "equal" || !a.amount ? [t.stepAmountUnknown] : []),
  ];
  return <>
    <RecoverNav />
